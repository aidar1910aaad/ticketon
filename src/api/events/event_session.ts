export async function fetchEventSessions(): Promise<any[]> {
    try {
      const response = await fetch("https://2.56.177.66:8444/api/event-sessions");
  ...
  export async function fetchEventSessionsByEvent(eventId: string): Promise<any[]> {
    try {
      const response = await fetch(`https://2.56.177.66:8444/api/event-sessions/search/by-event/${eventId}`);
  ...
  export async function updateEventSession(sessionId: string, sessionData: Record<string, any>, token: string): Promise<any> {
    try {
      const response = await fetch(`https://2.56.177.66:8444/api/event-sessions/${sessionId}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
