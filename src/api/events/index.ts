export async function fetchCategories(): Promise<{ id: string; name: string }[]> {
  try {
    const response = await fetch("https://2.56.177.66:8444/api/event-categories");
...
export async function createEvent(eventData: Record<string, any>, token: string): Promise<any> {
  try {
    ...
    const response = await fetch("https://2.56.177.66:8444/api/events", {
      method: "POST",
      headers: {
...
export async function deleteEvent(eventId: string, token: string): Promise<boolean> {
  try {
    const response = await fetch(`https://2.56.177.66:8444/api/events/${eventId}`, {
      method: "DELETE",
...
export async function fetchEventById(eventId: string): Promise<any> {
  try {
    const response = await fetch(`https://2.56.177.66:8444/api/events/search/by-id/${eventId}`);
